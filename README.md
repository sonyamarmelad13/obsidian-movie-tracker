...
## 📖 О проекте
Всем салют, хочу представить вам мою модель кинотеки для Obsidian, созданную с помощью плагина Dataview и Kinopoisk search. Изначально создавал чисто для себя, но может кому-то тоже будет полезно, поэтому решил выложить сюда.

## ✨ Возможности

### 🎬 Управление коллекцией
- Учёт фильмов и сериалов в одной таблице через теги `фильм` / `сериал`
- Интеграция с плагином Kinopoisk Search для добавления новых записей одной кнопкой
...
Создай заметку `Кинотека.md` в корне vault. Затем открой файл [`kinoteka.md`](https://github.com/sonyamarmelad13/obsidian-movie-tracker/blob/main/kinoteka.md?plain=1) в этом репозитории, скопируй всё его содержимое и вставь в свою заметку `Кинотека.md`.

[![Открыть kinoteka.md](https://img.shields.io/badge/📋_Открыть_код-CC0000?style=for-the-badge)](https://github.com/sonyamarmelad13/obsidian-movie-tracker/blob/main/kinoteka.md?plain=1)
...
- Папка `Фильмы` для заметок  
- Настроенный плагин Kinopoisk Search
- Дашборд `Кинотека.md` с Hero-картинкой

Открывай `Кинотека.md`, нажимай **+ ДОБАВИТЬ** и пополняй коллекцию.
...
A: Установи плагин **Kinopoisk Search** (см. шаг 1 установки). Если он установлен, но кнопка не находит команду — открой консоль (`Ctrl+Shift+I`) и проверь сообщения. Скрипт перебирает несколько ID команд автоматически.
