## 📖 О проекте
Всем салют, хочу представить вам мою модель кинотеки для Obsidiana, созданную с помощью плагина Dataview и Kinopoisk search. Изначально создавал чисто для себя, но может кому-то тоже будет полезно, поэтому решил выложить сюда. Код целиком и полностью написан с помощью Claude.
...
Создай заметку `Кинотека.md` и вставь содержимое файла [`kinoteka.md`](./kinoteka.md) из этого репозитория.
...
Эстетика проекта вдохновлена визуалом субкультуры ЧВК "Редан". Это просто стилизация под мрачную готическую эстетику для тех, кто любит тёмные темы и киберпанк-вайб. Используй с любовью к кино.
